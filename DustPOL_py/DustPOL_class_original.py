...
class DustPOL:
    ...
    @auto_refresh
    def cal_pol_abs(self,progress=False,save_output=False):
        '''
        ###-------------------------------------------------------------------------------##
    ...
    @auto_refresh
    def cal_pol_emi(self,tau=0.0,Tdust=None,progress=False,save_output=False):#,get_planck_option=True):
        '''
        ##-------------------------------------------------------------------------------##
    ...
    @auto_refresh
    def isoCloud_los(self,r0,Av_fixed_amax=False,fixed_amax_value=None,progress=False,save_output=False,filename_output=None):
        '''
        ##-------------------------------------------------------------------------------##
        ...
        self.update_radiation()
        self.get_coefficients_files() ##need to be here <-- grain size 
        ...
        for j in range(len(z_)):
            ...
            if (r_compute>max_radius):
                ...
                w_abs,dp_abs,dpmix_abs=self.cal_pol_abs(progress=progress)
                
                #emission polarization
                dIext_emi,dIp_emi,dIpmix_emi=self.cal_pol_emi(Tdust=self.Tdust,progress=progress)[1]
    ...
    def isoCloud_pos(self,Av_fixed_amax=False,fixed_amax_value=None,progress=False):
        ...
        ##-------------------------------------------------------------------------------##
        '''
        self.get_coefficients_files() ##redundant (because it's called in __init__), but just to be sure ...
        ...
        for i,r0 in enumerate(r0_range):
            ...
            w,NH_,Av_,Iext_,pabs,pemi=self.isoCloud_los(r0,Av_fixed_amax=Av_fixed_amax,fixed_amax_value=fixed_amax_value,progress=progress)
            Av_array.append(Av_)
